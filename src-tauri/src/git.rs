...
/// Run `git -C <vault> <args>`, returning stdout or stderr as the error.
...
/// Commit args with an identity fallback so commits never fail on machines
...
#[tauri::command]
pub fn git_status(vault: String) -> Result<Vec<GitFileStatus>, String> {
...
#[tauri::command]
pub fn git_log(vault: String, skip: u64, limit: u64) -> Result<Vec<CommitInfo>, String> {
    let skip_arg = format!("--skip={skip}");
    let n_arg = format!("-n{limit}");
    match run(&vault, &["log", &skip_arg, &n_arg, LOG_FORMAT, "--name-only"]) {
        Ok(out) => Ok(parse_log(&out)),
        // Fresh repos with no commits yet are not an error for the UI.
        Err(_) => Ok(vec![]),
    }
}

#[tauri::command]
pub fn git_file_log(vault: String, path: String, limit: u64) -> Result<Vec<CommitInfo>, String> {
    let n_arg = format!("-n{limit}");
...
#[tauri::command]
pub fn git_diff(vault: String, hash: String, path: Option<String>) -> Result<String, String> {
    match path {
...
#[tauri::command]
pub fn git_commit(vault: String, message: String) -> Result<bool, String> {
    commit_all(&vault, &message)
}

#[tauri::command]
pub fn git_has_remote(vault: String) -> Result<bool, String> {
    Ok(!run(&vault, &["remote"])?.trim().is_empty())
}

#[tauri::command]
pub fn git_sync(vault: String) -> Result<SyncResult, String> {
